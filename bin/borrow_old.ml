  ...
  let borrow_ck pipe_name =
    ...
    let rec drill cur_node borrow_map =
      ...
      | n -> (
          ...
          (* TODO! *)
          (* if pipe returns borrow, it must be of one of the arguments *)
          (* so we must check which lifetime is associated with the *)
          (* return type and for all possible returned arguments *)
          (* borrow them for ~at least~ that long *)

          (* this might require a bit of a re-work since *)
          (* we need to basically add borrows which last longer than the *)
          (* scope of the pipe call *)

          match n with
          | PipeCall (_, (n, _), _) ->
              (* todo: throw this in a map *)
              let _pipe_defn = List.find (fun p -> p.name = n) pipes in
